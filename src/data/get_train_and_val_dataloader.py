...
# Fixes issue with multithreading
...
def get_training_data_loader(
    ...
    val_transforms = transforms.Compose(
        [
            ...
            transforms.EnsureChannelFirstd(keys=["image"]) if is_grayscale else identity_transform,
            transforms.Lambdad(keys="image", func=select_first_channel) if is_grayscale else identity_transform, # needed for BRATs data with 4 modalities in 1
            central_crop_transform,
            resize_transform,
            transforms.ScaleIntensityd(keys=["image"], minv=0.0, maxv=1.0),
            transforms.RandFlipD(keys=["image"], spatial_axis=0, prob=1.0) if add_vflip else identity_transform,
            transforms.RandFlipD(keys=["image"], spatial_axis=1, prob=1.0) if add_hflip else identity_transform,
